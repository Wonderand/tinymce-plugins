...
(function () {
  ...
  function countVideos(content) {
    // 计算视频数量，假设使用的是 <video> 标签或嵌入的 iframe
    const videoMatches = content.match(/<video [^>]*>|<iframe [^>]*src=['"][^'"]+['"][^>]*>/g) || [];
  ...
  const get = editor => ({
    body: {
      ...
      getImageCount: () => countImages(editor.getContent()),
      getVideoCount: () => countVideos(editor.getContent())
    },
    selection: {
      ...
      getImageCount: () => countImages(editor.selection.getContent()),
      getVideoCount: () => countVideos(editor.selection.getContent())
    },
      ...
      body: {
        ...
        items: [{
          ...
          cells: [
            ...
            [
              '图片',
              String(api.body.getImageCount()),
              String(api.selection.getImageCount())
            ],
            [
              '视频',
              String(api.body.getVideoCount()),
              String(api.selection.getVideoCount())
            ]
          ]
        }]
      },
