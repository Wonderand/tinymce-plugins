...
(function () {
  ...
  const getPreviewHtml = editor => {
    var str = editor.getContent({ format: 'raw' });
    // 创建一个新的DOM文档片段
    var docFragment = document.createElement('div');
    // 将HTML字符串安全地插入到文档片段中
    docFragment.innerHTML = str;
    // 选择所有的 `mjx-container` 元素
    var containers = docFragment.querySelectorAll('mjx-container');
    // 遍历这些元素并移除 `font-size` 属性
    ...
    const previewHtml = '<!DOCTYPE html>' + '<html>' + '<head>' + headHtml + '</head>' + '<body id="' + encode(bodyId) + '" class="mce-content-body ' + encode(bodyClass) + '"' + dirAttr + '>' + modifiedStr + preventClicksOnLinksScript + '</body>' + '</html>';
    return previewHtml;
  };
